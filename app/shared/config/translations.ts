const en = {
  translation: {
    ...
    apiDocs: {
      ...
      quickStart: "Quick start", requestHint: "Use POST with a JSON body. GET with X-Data containing Base64-encoded UTF-8 JSON is also available. X-Language selects localized API errors.", getRequest: "GET (additional method)", postRequest: "POST (recommended)",
...
const ru: Dictionary = { translation: {
  ...
  apiDocs: {
    ...
    quickStart: "Быстрый старт", requestHint: "Используйте POST с JSON-телом. Также доступен GET с X-Data, содержащим UTF-8 JSON в Base64. X-Language задаёт язык ошибок API.", getRequest: "GET (дополнительный метод)", postRequest: "POST (рекомендуется)",
...
const de: Dictionary = { translation: {
  ...
  apiDocs: {
    ...
    quickStart: "Schnellstart", requestHint: "Verwenden Sie POST mit JSON-Inhalt. GET mit X-Data, das Base64-kodiertes UTF-8-JSON enthält, ist ebenfalls verfügbar. X-Language wählt lokalisierte API-Fehler.", getRequest: "GET (zusätzliche Methode)", postRequest: "POST (empfohlen)",
...
const fr: Dictionary = { translation: {
  ...
  apiDocs: {
    ...
    quickStart: "Démarrage rapide", requestHint: "Utilisez POST avec un corps JSON. GET avec X-Data contenant du JSON UTF-8 encodé en Base64 est également disponible. X-Language sélectionne les erreurs API localisées.", getRequest: "GET (méthode supplémentaire)", postRequest: "POST (recommandé)",
...
const es: Dictionary = { translation: {
  ...
  apiDocs: {
    ...
    quickStart: "Inicio rápido", requestHint: "Usa POST con un cuerpo JSON. GET con X-Data que contiene JSON UTF-8 codificado en Base64 también está disponible. X-Language selecciona los errores de API localizados.", getRequest: "GET (método adicional)", postRequest: "POST (recomendado)",
